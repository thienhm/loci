...
The AI agent interacts with Loci via `loci` CLI commands using the Bash tool. The server does **not** need to be running — the CLI reads directly from disk.

| Operation | Command |
|---|---|
| List tickets | `loci list --json` |
| Get ticket + docs | `loci get <id> --json` |
...
Set assignee to null: `--assignee null`
